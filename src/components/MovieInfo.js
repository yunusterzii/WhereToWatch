...
const MovieInfo = () => {
    ...
    const FetchInfo = async () => {
        ...
        setMovieInfo(
            <Container fluid style={{width: "100vw", justifyContent: "center"}} className="mx-4 d-flex">
                <Card style={{width : "60vw", border : "none", background : "rgb(217, 228, 241)"}}>
                    <Card.Body>
                        <Row style={{background : "rgb(255,255,255)", borderRadius : "1em"}}>
                            ...
                            <Col style={{marginLeft : "1em"}} cs={8}>
                                <Card.Title>{element.title}</Card.Title>
                                <Card.Text style={{marginTop : "2em"}}>{element.overview}</Card.Text>
                                <Card.Text style={{display : "flex"}}>{genres}</Card.Text>
                                <Card.Text style={{marginTop : "0.5em"}} className="d-flex">
                                    <div style={{display: "inline-block", marginRight : "1em"}}>{element.runtime + " mins"}</div>
                                    <div style={{display: "inline-block", marginRight : "1em"}}>{element.release_date}</div>
                                </Card.Text>
                            </Col>
                        </Row>
                    </Card.Body>
    ...
    const FetchCredits = async () => {
        ...
        (<Card style={{display: "inline-block", marginRight : "1em", marginTop : "2em", width : "14em", height : "26em", padding : "0"}} key={key}>
            <Card.Img src={"https://image.tmdb.org/t/p/original/" + element.profile_path}></Card.Img>
            <Card.Body>
                <Card.Text style={{marginBottom : "0", fontWeight : "bold"}}>{element.name}</Card.Text>
                <Card.Text style={{marginTop : "0", fontSize : "0.75em"}}>{element.character}</Card.Text>
            </Card.Body>
        </Card>)
        : null
        ))
    ...
    const FetchRecommendations = async () => {
        ...
        setMovieRecommends(recommendations.map((element, key) => 
            <Card key={key} style={{display : "inline-block", marginRight : "1em", marginTop : "1em",width : "19.5em", height : "16em"}}>
                <Card.Body>
                    <Row>
                        <Col style={{padding : "0"}} xs={4}>
                        ...
                        <Col cs={8}>
                            <Card.Title><Link to={"/movie/" + element.title + "&id=" + element.id} ScrollReset={true} style={{color: "black", textDecoration: "none"}}>{element.title}</Link></Card.Title>
                            <Card.Text>{element.release_date.split("-")[0]}</Card.Text>
                            <div style={{width : "8em", height : "7.5em", overflow : "hidden"}}> {element.overview}
                            </div>
                        </Col>
                    </Row>
                </Card.Body>
    ...
    return(
        <Container name="top" fluid style={{background : "rgb(217, 228, 241)"}}>
